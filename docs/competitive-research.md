...
4. **主站站点数据的备份/导出** —— 屿事已做导出导入，但主站的 SITES 收藏数据没有对应能力，数据资产不对称。
...
1. **自然语言输入** —— Todoist 的核心差异化能力：输入"明天 10:00 开会 !1 #工作"自动解析出时间、优先级、标签。屿事目前只能逐字段填，是最明显的体验差距，且实现成本可控（正则解析中文/英文日期 + `!n` + `#tag`）。
2. **重复任务** —— 每日/每周/自定义周期。屿事缺这个，用户每天要手动重复添加。
3. **完成率统计面板** —— "本周完成 X 项 / 连续 X 天打卡 / 完成率趋势"。屿事有实时计数，但没有趋势与成就。
...
| 项 | 归属 | 依据 |
|---|---|---|
| 屿咪 × 屿事联动（读 todos，进度 + 表情 + 台词） | 屿咪 | 唯一无竞品对标的差异化；把"陪伴"落到真实功能数据上 |
| 屿事：自然语言输入（日期/优先级/标签解析） | 屿事 | Todoist 标杆能力；屿事最明显的体验差距 |
| 屿事：完成率/统计面板（本周完成数、连续打卡） | 屿事 | 直接对齐"41% 待办永不完成"这个品类核心矛盾 |
| 主站：书签一键导入 + 站点数据备份导出 | 屿航 | 导航站用户最大门槛；与屿事能力对称 |
